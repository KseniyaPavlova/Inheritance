...
namespace Geometry
{
	//enum (Enumeration - Перечисление) - этонабор целочисленных констант
// Перечисление также является типом данных
	enum Color
	{
		red = 0x000000FF,
		green = 0x0000FF00,
	...
	public:
		...
		void draw()const
		{
			// В переменную hConsole сохраням страндартный вывод текущего окна консоли
			HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
			SetConsoleTextAttribute(hConsole, get_color()); //Для стандартного вывода текущего окна задаем цвет текста и фона
	...
	public:
		...
		{
			cout << typeid(*this).name() << endl;
			cout << "Cторона треугольника: " << /*get_*/side/*()*/ << endl;
			cout << "Высота треугольника: " << get_height() << endl;
			cout << "Площадь треугольниика " << get_area() << endl;
			cout << "Периметр треугольниика " << get_perimeter() << endl;
			Shape::info();
		}
	};
	...
	public:
		...
		void info()const
		{
			...
			cout << "Cторона треугольника: " << side << endl;
			cout << "Высота треугольника: " << get_height() << endl;
			//cout << "Площадь треугольниика " << get_area() << endl;
			//cout << "Периметр треугольниика " << get_perimeter() << endl;
			Shape::info();
